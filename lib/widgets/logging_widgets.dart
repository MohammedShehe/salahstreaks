...
class _LoggingWidgetState extends State<LoggingWidget> {
              ...
              return Container(
                ...
                child: ListTile(
                  leading: Container(
                    ...
                    child: Icon(
                      prayerIcons[index],
                      color: isLogged ? Colors.white : AppThemeColors.textSecondary(context),
                      size: 20,
                    ),
                  ),
                  title: Text(
                  ...
                  subtitle: Text(
                    prayerTimes[index],
                    style: TextStyle(
                      color: isLogged ? Colors.green[300] : AppThemeColors.textHint(context),
                      fontSize: 12,
                    ),
                  ),
                  trailing: isLogged
  ...
  List<Widget> _buildSawmFields() {
          ...
          child: Column(
            children: [
              ...
              Text(
                ...
                style: TextStyle(color: AppThemeColors.textPrimary(context), fontSize: 16, fontWeight: FontWeight.w600),
              ),
              Text(
                'Come back tomorrow to log again',
                style: TextStyle(color: AppThemeColors.textSecondary(context), fontSize: 14),
              ),
            ],
          ),
        ),
    ...
    return [
      Text('Select Fasting Type:', style: TextStyle(color: AppThemeColors.textPrimary(context), fontSize: 16)),
      const SizedBox(height: 12),
      Row(
        children: [
          Expanded(
            child: RadioListTile<String>(
              title: Text('Fardh (Ramadan)', style: TextStyle(color: AppThemeColors.textPrimary(context))),
              value: 'Fardh',
          ...
          Expanded(
            child: RadioListTile<String>(
              title: Text('Sunnah', style: TextStyle(color: AppThemeColors.textPrimary(context))),
              value: 'Sunnah',
              groupValue: _sawmType,
              onChanged: (value) => setState(() => _sawmType = value!),
              activeColor: Colors.green,
  ...
  List<Widget> _buildQiyyamFields() {
    ...
    return [
      Text('Tahajjud Rak\'ah:', style: TextStyle(color: AppThemeColors.textPrimary(context), fontSize: 16)),
      const SizedBox(height: 8),
      Wrap(
        spacing: 8,
        children: [2, 4, 6, 8, 10].map((rakah) {
          return ChoiceChip(
            ...
            labelStyle: TextStyle(
              color: _tahajjudRakah == rakah ? Colors.white : AppThemeColors.textSecondary(context),
            ),
          );
        }).toList(),
      ),
      const SizedBox(height: 20),
      Text('Witr Rak\'ah:', style: TextStyle(color: AppThemeColors.textPrimary(context), fontSize: 16)),
      const SizedBox(height: 8),
      Wrap(
        spacing: 8,
        children: [1, 3, 5].map((rakah) {
          return ChoiceChip(
            ...
            backgroundColor: Colors.grey[800],
            labelStyle: TextStyle(
              color: _witrRakah == rakah ? Colors.white : AppThemeColors.textSecondary(context),
            ),
          );
        }).toList(),
      ),
    ...
    return [
      Container(
        ...
        child: Column(
          children: [
            Text(
              'Log by:',
              style: TextStyle(color: AppThemeColors.textPrimary(context), fontSize: 14),
            ),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(
                  child: ChoiceChip(
                    ...
                    backgroundColor: Colors.grey[800],
                    labelStyle: TextStyle(
                      color: _quranLogMode == 'Verses' ? Colors.white : AppThemeColors.textSecondary(context),
                    ),
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: ChoiceChip(
                    ...
                    backgroundColor: Colors.grey[800],
                    labelStyle: TextStyle(
                      color: _quranLogMode == 'Pages' ? Colors.white : AppThemeColors.textSecondary(context),
                    ),
                  ),
                ),
              ],
      ...
      DropdownButtonFormField<String>(
        ...
        items: quranSurahList.map((surah) {
          return DropdownMenuItem(
            value: surah,
            child: Text(surah, style: TextStyle(color: AppThemeColors.textPrimary(context))),
          );
        }).toList(),
      ...
      if (_quranLogMode == 'Pages') ...[
        ...
        Container(
          ...
          child: Text(
            '💡 1 Juz = 20 pages',
            style: TextStyle(color: AppThemeColors.textSecondary(context), fontSize: 12),
          ),
        ),
      ],
    ];
  ...
  List<Widget> _buildSadaqatFields() {
    ...
    return [
      Text('Type of Sadaqah:', style: TextStyle(color: AppThemeColors.textPrimary(context), fontSize: 16)),
      const SizedBox(height: 8),
      Wrap(
        spacing: 8,
        children: ['Money', 'Food', 'Other'].map((type) {
          return ChoiceChip(
            ...
            backgroundColor: Colors.grey[800],
            labelStyle: TextStyle(
              color: _sadaqatType == type ? Colors.white : AppThemeColors.textSecondary(context),
            ),
          );
        }).toList(),
      ),
  ...
  void _submitLog() {
    ...
    // For Salah, individual prayers are toggled live via toggleSalah.
    // The submit button just closes the sheet — calling logIbadat('Salah')
    // was a no-op (logIbadat intentionally ignores Salah) so the snackbar
    // lied about success while nothing was saved.
    if (widget.type == 'Salah') {
      final count = _salahTicked.values.where((v) => v).length;
      if (count == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
    ...
    if (widget.type == 'Quran') {
      if (_quranLogMode == 'Verses') {
        ...
        // Approximate pages from verses so Quran progress still advances.
        quranPagesRead = (versesCount / 20).ceil().clamp(1, 600);
      } else {
        final pagesCount = _pageEnd - _pageStart + 1;
        if (pagesCount <= 0) {
          ScaffoldMessenger.of(context).showSnackBar(
